...
test.describe('Settings (signed in)', () => {
  ...
  test('renders hero, profile card, and the five section toggles', async ({ page }) => {
    await expect(page.getByText('Your settings.')).toBeVisible({ timeout: 20_000 });
...
await expect(page.getByText('Purchases & Sales', { exact: true })).toBeVisible();
  ...
  test('Purchases & Sales section expands to show its rows', async ({ page }) => {
    ...
    await expect(page.getByText('Bundle discount', { exact: true })).toBeVisible();
    await expect(page.getByText('Vacation mode', { exact: true })).toBeVisible();
    await expect(page.getByText('Share your profile', { exact: true })).toBeVisible();
  });

  test('Verification section expands and shows the three rows', async ({ page }) => {
    await page.getByText('Verification, payouts & shipping', { exact: true }).click();
  ...
  test('Manage account section shows the email row + change-password / delete actions', async ({ page }) => {
  ...
  test('Help center section shows the support / terms / privacy rows', async ({ page }) => {
