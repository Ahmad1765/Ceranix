...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!stripeSecret) return json({ error: 'STRIPE_SECRET_KEY not set' }, 500);
    ...
    if (!returnUrl) return json({ error: 'return_url required' }, 400);
    ...
    if (!res.ok) {
      return json({ error: stripeBody?.error?.message ?? 'Stripe error' }, 500);
    }
