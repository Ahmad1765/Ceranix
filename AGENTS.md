...
- **Mobile Safari Viewport & Keyboard Sync:** Never use naive `position: fixed; top: 0; bottom: 0;` on web without syncing to `window.visualViewport.offsetTop` and `window.visualViewport.height`. Always use `<SafeContainer mode="keyboard-avoiding">` to prevent Safari from scrolling headers off-screen and leaving blank gaps above the keyboard.
