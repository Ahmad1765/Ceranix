# Ceranix — What We're Building (Plain English)
...
Think of Ceranix like a combination of **Depop + WhatsApp + a small online mall**.
...
### 6. The Score Refresher *(runs automatically every 30 minutes)*
**Job:** Updates the ranking score for every live listing so the feed stays fresh and relevant.

### 7. The Offer Expiry Checker *(runs automatically every 5 minutes)*
...
- **Data Rules**: Every piece of data has rules — for example, you can only read *your own* orders, not someone else's. This is enforced at the database level, not just in the app code.
...
In short, we're building a reliable, secure, and scalable engine that powers every tap, swipe, and transaction in the Ceranix app. The frontend already looks great — now we're wiring up the brain behind it.
...
*Plain English version of BACKEND_BLUEPRINT.md — for questions, refer back to the technical document.*
