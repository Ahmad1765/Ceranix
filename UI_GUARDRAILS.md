...
| **Filter / Controls Icon** | Vertical sliders (`Feather` `sliders`), generic funnels (`filter`), or text labels like "Filter" / "Filters" beside the icon. | **Horizontal Slider Trio & Icon-Only Rule:** All filter buttons and filter chips across the application (Home header `FeedSearch`, Discover `SearchFilterChips`, search overlays) **MUST** use the canonical `<FilterSlidersIcon>` (`@/components/ui/FilterSlidersIcon`) and **MUST NOT** include the word "Filter" or "Filters". The control renders strictly as an icon-only pill or circular container matching the quiet atelier aesthetic. Features 3 horizontal tracks with staggered circular slider knobs (top-left, middle-right, bottom-left) and rounded stroke caps, derived from `assets/images/slider-horizontal-svgrepo-com.svg` rotated horizontally. Knob centers remain hollow (`fill="none"`) so the button fill (Paper White at rest, Solid Ink when active) shows cleanly through. |
...
| `components/ui/FilterSlidersIcon.tsx` | Visual Identity / Controls | Canonical filter icon across the entire app. Source asset: `assets/images/slider-horizontal-svgrepo-com.svg` (rotated horizontally: top-left, middle-right, bottom-left). Matches quiet atelier pill controls. |
